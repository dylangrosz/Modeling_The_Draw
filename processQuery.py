...
def processTrends(h_data, gender, typeCol, resID, simple, des_year):
    ...
    if len(cutoffs) == 1:
        ...
        return coef[0] * des_year + coef[1]

def processSingleQuery(query, simple=True):
    ...
    if resID in h_data["res_name_edited"].unique():
        ...
        cutoff = processTrends(h_data, gender, typeCol, resID, simple, 2019)
        # if simple:
...
q = sys.argv
output = processSingleQuery(q, simple=False)